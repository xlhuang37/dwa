...
trunk_get_filter_id(trunk_handle *spl) {
   // clang-format off
   uint64 filter_id = __sync_fetch_and_add(&spl->filter_id, 1);
   return filter_id;
}

static inline void
trunk_init_filter_id(trunk_handle *spl, uint64 stored_filter_id) {
   platform_assert(spl->filter_id == 0);
   // clang-format off
   __sync_fetch_and_add(&spl->filter_id, stored_filter_id);
}

static inline uint64
trunk_get_log_gen_id(trunk_handle *spl) {
   // clang-format off
   uint64 log_gen_id = __sync_fetch_and_add(&spl->log_gen_id, 1);
   return log_gen_id;
}

static inline void
trunk_init_log_gen_id(trunk_handle *spl, uint64 stored_log_gen_id) {
   platform_assert(spl->log_gen_id == 0);
   // clang-format off
   __sync_fetch_and_add(&spl->log_gen_id, stored_log_gen_id);
}
