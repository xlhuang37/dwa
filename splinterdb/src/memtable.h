...
typedef struct memtable_context {
   ...
   page_reference  insert_lock_ref;
   ...
   // to modify.
   page_reference  lookup_lock_ref;
...
memtable_verify(cache *cc, memtable *mt)
{
   ...
   return btree_verify_tree(cc, mt->cfg, &ref, PAGE_TYPE_MEMTABLE);
}
...
{
   page_reference ref = {.addr = mt->root_addr};
   btree_print_tree(log_handle, cc, mt->cfg, &ref, PAGE_TYPE_MEMTABLE);
}
...
{
   page_reference ref;
   ref.addr = mt->root_addr;
   btree_print_tree_stats(log_handle, cc, mt->cfg, &ref, PAGE_TYPE_MEMTABLE);
};
