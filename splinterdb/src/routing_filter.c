 ...
 *----------------------------------------------------------------------
 */
typedef struct ONDISK routing_bucket_hdr {
   uint16 num_remainders;
   char   encoding[];
} routing_bucket_hdr;

#define MAX_BUCKETS_PER_PAGE (10)

typedef struct filter_page_hdr {
   char   sig[HASH_SIZE];
   uint64 filter_id;
   uint64 bucket_id;
   uint8  num_buckets_in_page;
   uint16 bucket_offsets[MAX_BUCKETS_PER_PAGE];
} filter_page_hdr;

static inline void
init_filter_page_header(filter_page_hdr *filter_hdr, uint64 id, uint64 num_buckets)
{
   filter_hdr->filter_id = id;;
   filter_hdr->bucket_id = num_buckets;
   filter_hdr->num_buckets_in_page = 0;
   memset(filter_hdr->bucket_offsets, 0, sizeof(uint16)*MAX_BUCKETS_PER_PAGE);
}
...
static inline routing_bucket_hdr *
                   ...
                   page_handle   **filter_page)
{
   ...
   page_reference ref = {.addr = index_addr};
   page_handle *index_page = cache_get_no_auth(cc, &ref, TRUE, PAGE_TYPE_FILTER);
   ...
   page_reference ref1 = {.addr = header_addr};
   *filter_page                = cache_get_self_auth(cc, &ref1, TRUE, PAGE_TYPE_FILTER, NULL);
   filter_page_hdr *filter_hdr = (filter_page_hdr *)((*filter_page)->data);
   // (1) verify filter_id matches
   platform_assert(filter_hdr->filter_id == filter_id);
   // (2) verify desired bucket is correct
   if (!(index >= filter_hdr->bucket_id && index < filter_hdr->bucket_id + filter_hdr->num_buckets_in_page)) {
      platform_default_log("index=%ld, filter_hdr->bucket_id=%ld, num_buckets_in_page=%d\n",
                            index, filter_hdr->bucket_id, filter_hdr->num_buckets_in_page);
      platform_assert(0);
   }
   // (3) verify signature is correct
   //char buf[HASH_SIZE] = {0};
   //trunk_hmac((char*)((*filter_page)->data) + HASH_SIZE,
   //           cache_config_page_size(cfg->cache_cfg) - HASH_SIZE,
   //           buf, HASH_SIZE);
   //if (0 != memcmp(buf, filter_hdr->sig, HASH_SIZE)) {
   //   platform_default_log("signature does not match for filter page");
   //   platform_assert(0);
   //}
   uint64       header_off = hdr_raw_addr - header_addr;
   routing_bucket_hdr *hdr = (routing_bucket_hdr *)((*filter_page)->data + sizeof(filter_page_hdr) + header_off);
...
routing_header_length(routing_config *cfg, routing_bucket_hdr *hdr)
{
   uint64 metamessage_size =
      (hdr->num_remainders + cfg->index_size - 1) / 8 + 4;
   return metamessage_size + sizeof(routing_bucket_hdr);
}

static inline void
routing_unlock_and_unget_page(cache *cc, page_handle *page, bool is_filter_page)
{
   if (is_filter_page) {
      // copy the signature from the page to clockcache_entry
      memcpy(cache_get_page_hash(cc, page), page->data, HASH_SIZE);
   }
...
void
routing_get_bucket_counts(routing_config *cfg, routing_bucket_hdr *hdr, uint32 *count)
{
   uint64  start = 0;
   uint64  end;
   uint64  i;
...
routing_filter_add(cache           *cc,
                   ...
                   uint64           new_filter_id,
                   uint32          *new_fp_arr,
                   uint64           num_new_fp,
                   uint16           value)
{
   ZERO_CONTENTS(filter);
   filter->filter_id = new_filter_id;
   ...
   init_filter_page_header(filter_hdr, filter->filter_id, num_buckets);

   uint32 filter_page_header_size = sizeof(filter_page_hdr);
   char        *filter_cursor = filter_page->data + filter_page_header_size;
   ...
   for (uint32 old_index_no = 0; old_index_no < old_num_indices; old_index_no++)
   {
      ...
      if (old_filter->addr != 0) {
         routing_bucket_hdr *old_hdr = routing_get_header(
            cc, cfg, old_filter->filter_id, old_filter->addr, old_index_no, &old_filter_node);
      ...
      for (uint32 index_off = 0; index_off < new_indices_per_old_index;
         ...
         uint32 header_size   = encoding_size + sizeof(routing_bucket_hdr);
         if (header_size + remainder_block_size + filter_page_header_size > bytes_remaining_on_page) {
            trunk_hmac(filter_page->data + HASH_SIZE,
                       cache_config_page_size(cfg->cache_cfg) - HASH_SIZE,
                       filter_hdr->sig, HASH_SIZE, PAGE_TYPE_FILTER);
            routing_unlock_and_unget_page(cc, filter_page, TRUE);
            addr        = mini_alloc(&mini, 0, NULL_SLICE, NULL);
            filter_page = cache_alloc(cc, addr, PAGE_TYPE_FILTER);
            filter_hdr = (filter_page_hdr *)filter_page->data;
            filter_hdr->filter_id       = filter->filter_id;
            init_filter_page_header(filter_hdr, filter->filter_id, num_buckets);
            bytes_remaining_on_page = page_size;
            filter_cursor           = filter_page->data + filter_page_header_size;
         }
         ...
         uint64 filter_page_offset = filter_cursor - filter_page->data - filter_page_header_size;
         *index_cursor             = addr + filter_page_offset;

         num_buckets += 1;
         filter_hdr->num_buckets_in_page += 1;
         if (filter_hdr->num_buckets_in_page > MAX_BUCKETS_PER_PAGE) {
            platform_default_log("%s: filter_hdr->num_buckets_in_page=%d\n", __func__, filter_hdr->num_buckets_in_page);
            platform_default_log("%s: MAX_BUCKETS_PER_PAGE=%d\n", __func__, MAX_BUCKETS_PER_PAGE);
            platform_assert(0);
         }
         filter_hdr->bucket_offsets[filter_hdr->num_buckets_in_page-1] = filter_page_offset;

         routing_bucket_hdr *hdr    = (routing_bucket_hdr *)filter_cursor;
   ...
   for (uint64 i = 0; i < pages_per_extent; i++) {
      routing_unlock_and_unget_page(cc, index_page[i], FALSE);
   }
                        ...
                        uint64          num_indices)
{
   ...
   for (uint64 index_page_no = 0; index_page_no < num_index_pages;
      ...
      page_handle *index_page =
         cache_get_no_auth(cc, &ref, TRUE, PAGE_TYPE_FILTER);
   ...
   for (uint64 i = 0; i != num_filters; i++) {
      ...
      if (num_indices >= 16) {
         ...
         for (uint32 index_no = 0; index_no < num_indices; index_no++) {
            ...
            routing_bucket_hdr *hdr = routing_get_header(
               cc, cfg, filter[i].filter_id, filter[i].addr, index_no, &filter_node);
                      ...
                      uint64         *found_values)
{
   ...
   page_handle *filter_node;
   routing_bucket_hdr *hdr =
      routing_get_header(cc, cfg, filter->filter_id, filter->addr, index, &filter_node);
   uint64 encoding_size = (hdr->num_remainders + index_size - 1) / 8 + 4;
   uint64 header_length = encoding_size + sizeof(routing_bucket_hdr);
   ...
   do {
      switch (ctxt->state) {
         ...
         case routing_async_state_get_filter:
         {
            ...
            page_reference ref = { .addr = ctxt->page_addr };
            // Need to work on get_async sooner or later
            res = cache_get_async(
               cc, &ref, PAGE_TYPE_FILTER, cache_ctxt);
            switch (res) {
               case async_locked:
               case async_no_reqs:
         ...
         case routing_async_state_got_filter:
         {
            ...
            routing_bucket_hdr *hdr =
               (routing_bucket_hdr *)(cache_ctxt->page->data + sizeof(filter_page_hdr) +
                               + (ctxt->header_addr
                                  % cache_config_page_size(cfg->cache_cfg)));
...
void
routing_filter_print_encoding(routing_config *cfg, routing_bucket_hdr *hdr)
{
   uint32 i;
...
routing_filter_print(cache *cc, routing_config *cfg, routing_filter *filter)
{
   ...
   for (i = 0; i < num_indices; i++) {
      ...
      page_handle *filter_page;
      routing_bucket_hdr *hdr =
         routing_get_header(cc, cfg, filter->filter_id, filter_addr, i, &filter_page);
      routing_filter_print_encoding(cfg, hdr);
