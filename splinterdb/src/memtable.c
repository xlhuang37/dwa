...
{
   cache *cc        = ctxt->cc;
   page_reference lock_ref = ctxt->insert_lock_ref;
   uint64 wait      = 100;
   while (TRUE) {
      *lock_page      = cache_get(cc, &lock_ref, TRUE, PAGE_TYPE_LOCK_NO_DATA, NULL);
...
{
   return cache_get(
      ctxt->cc, &ctxt->lookup_lock_ref, TRUE, PAGE_TYPE_LOCK_NO_DATA, NULL);
}
...
memtable_dec_ref_maybe_recycle(memtable_context *ctxt, memtable *mt)
{
   ...
   if (freed) {
      ...
      page_reference ref = btree_create(cc, mt->cfg, &mt->mini, PAGE_TYPE_MEMTABLE);
      mt->root_addr = ref.addr;
...
memtable_force_finalize(memtable_context *ctxt)
{
   ...
   cache       *cc        = ctxt->cc;
   page_handle *lock_page =
      cache_get(cc, &lock_ref, TRUE, PAGE_TYPE_LOCK_NO_DATA, NULL);
   uint64 wait = 100;
                        ...
                        void            *process_ctxt)
{
   ...
   ctxt->insert_lock_ref.addr = base_addr;
   ctxt->lookup_lock_ref.addr = base_addr + cache_page_size(cc);

   page_handle *lock_page =
      cache_alloc(cc, ctxt->insert_lock_ref.addr, PAGE_TYPE_LOCK_NO_DATA);
   cache_pin(cc, lock_page);
   cache_unlock(cc, lock_page);
   cache_unclaim(cc, lock_page);
   cache_unget(cc, lock_page);

   lock_page = cache_alloc(cc, ctxt->lookup_lock_ref.addr, PAGE_TYPE_LOCK_NO_DATA);
   cache_pin(cc, lock_page);
   cache_unlock(cc, lock_page);
   cache_unclaim(cc, lock_page);
   cache_unget(cc, lock_page);
...
memtable_context_destroy(platform_heap_id hid, memtable_context *ctxt)
{
   ...
   allocator *al = cache_allocator(cc);
   uint8      ref =
      allocator_dec_ref(al, ctxt->insert_lock_ref.addr, PAGE_TYPE_LOCK_NO_DATA);
   platform_assert(ref == AL_NO_REFS);
   cache_hard_evict_extent(cc, ctxt->insert_lock_ref.addr, PAGE_TYPE_LOCK_NO_DATA);
   ref = allocator_dec_ref(al, ctxt->insert_lock_ref.addr, PAGE_TYPE_LOCK_NO_DATA);
