...
typedef page_handle *(*page_get_no_auth_fn)(cache    *cc,
                                            page_reference *ref,
                                            bool      blocking,
                                            page_type type);

typedef int (*page_hash_fn)(cache *cc, 
                            page_handle *page,
                            uint64 addr
                           );

typedef void (*fill_page_ref_fn)(cache *cc,
                                 char *ref,
                                 page_handle *page);

typedef char* (*get_page_hash_fn)(cache *cc,
                                  page_handle *page);

typedef void (*print_page_hash_fn)(cache *cc,
                                   page_handle *page,
                                   int line);
...
typedef bool (*page_valid_fn)(cache *cc, page_reference *ref);
...
typedef struct cache_ops {
   ...
   page_get_no_auth_fn  page_get_no_auth;
   page_get_fn          page_get_selfauth;
   ...
   fill_page_ref_fn     fill_page_ref;
   get_page_hash_fn     get_page_hash;
   print_page_hash_fn   print_page_hash;
} cache_ops;
...
static inline page_handle *
cache_get(cache *cc, page_reference *ref, bool blocking, page_type type, bool *succeed)
{
   //if (enable_print) {
   //   platform_default_log("%s cache_get ref->addr=%lu\n", __func__, ref->addr);
   //}
   return cc->ops->page_get(cc, ref, blocking, type, succeed);
}

static inline page_handle *
cache_get_no_auth(cache *cc, page_reference *ref, bool blocking, page_type type)
{
   //if (enable_print) {
   //   platform_default_log("%s cache_get ref->addr=%lu\n", __func__, ref->addr);
   //}
   return cc->ops->page_get_no_auth(cc, ref, blocking, type);
}

static inline page_handle *
cache_get_self_auth(cache *cc, page_reference *ref, bool blocking, page_type type, bool *succeed)
{
   //if (enable_print) {
   //   platform_default_log("%s cache_get ref->addr=%lu\n", __func__, ref->addr);
   //}
   return cc->ops->page_get_selfauth(cc, ref, blocking, type, succeed);
}
...
cache_get_async(cache *cc, page_reference *ref, page_type type, cache_async_ctxt *ctxt)
{
   return cc->ops->page_get_async(cc, ref, type, ctxt);
}
...
static inline bool
cache_page_valid(cache *cc, page_reference *ref)
{
   return cc->ops->page_valid(cc, ref);
}
...
static inline int
cache_hash(cache *cc, page_handle *page, uint64 addr)
{
   return cc->ops->page_hash(cc, page, addr);
}

static inline void
cache_fill_page_reference(cache *cc, char *ref, page_handle *page)
{
   return cc->ops->fill_page_ref(cc, ref, page);
}

static inline char *
cache_get_page_hash(cache *cc, page_handle *page)
{
   return cc->ops->get_page_hash(cc, page);
}

//static inline void
//cache_print_page_hash(cache *cc, page_handle *page, int line)
//{
//   return cc->ops->print_page_hash(cc, page, line);
//}
#endif // __CACHE_H
