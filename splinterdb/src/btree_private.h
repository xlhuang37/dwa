...
struct ONDISK btree_hdr {
   page_reference      next_ref;
   uint64      next_extent_addr;
   // Only the root node will persist this information
...
_Static_assert(sizeof(index_entry)
                  == sizeof(uint64) + HASH_SIZE * sizeof(char) + 3 * sizeof(uint32)
...
static inline page_reference
index_entry_child_ref(const index_entry *entry)
{
   debug_assert(entry->key_indirect == FALSE);
   return entry->pivot_data.child_ref;
}
...
static inline page_reference
btree_get_child_ref(const btree_config *cfg,
                     const btree_hdr    *hdr,
                     table_index         k)
{
   return index_entry_child_ref(btree_get_index_entry(cfg, hdr, k));
}
