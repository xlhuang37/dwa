...
page_handle *
clockcache_get(clockcache *cc, page_reference *ref, bool blocking, page_type type, bool *succeed);

page_handle *
clockcache_get_no_auth(clockcache *cc, page_reference *ref, bool blocking, page_type type);

page_handle *
clockcache_get_self_auth(clockcache *cc, page_reference *ref, bool blocking, page_type type, bool *succeed);
...
bool
clockcache_page_valid(clockcache *cc, page_reference *ref);
...
int
clockcache_page_hash(cache *cc, page_handle *page, uint64 addr);

void
clockcache_fill_page_reference(cache *cc, char *ref, page_handle *page);

char *
clockcache_get_page_hash(cache *cc, page_handle *page);

void
clockcache_print_page_hash(cache *cc, page_handle *page, int line);
...
page_handle *
clockcache_get_virtual(cache *c, page_reference *ref, bool blocking, page_type type, bool *succeed)
{
   clockcache *cc = (clockcache *)c;
   return clockcache_get(cc, ref, blocking, type, succeed);
}

page_handle *
clockcache_get_virtual_no_auth(cache *c, page_reference *ref, bool blocking, page_type type)
{
   clockcache *cc = (clockcache *)c;
   return clockcache_get_no_auth(cc, ref, blocking, type);
}

page_handle *
clockcache_get_virtual_self_auth(cache *c, page_reference *ref, bool blocking, page_type type, bool *succeed)
{
   clockcache *cc = (clockcache *)c;
   return clockcache_get_self_auth(cc, ref, blocking, type, succeed);
}
...
{
   clockcache *cc = (clockcache *)c;
   return clockcache_get_async(cc, ref, type, ctxt);
}
...
bool
clockcache_page_valid_virtual(cache *c, page_reference *ref)
{
   clockcache *cc = (clockcache *)c;
   return clockcache_page_valid(cc, ref);
}
...
static cache_ops clockcache_ops = {
   ...
   .page_get_no_auth  = clockcache_get_virtual_no_auth,
   .page_get_selfauth = clockcache_get_virtual_self_auth,
   ...
   .page_hash         = clockcache_page_hash,
   .fill_page_ref     = clockcache_fill_page_reference,
   .get_page_hash     = clockcache_get_page_hash,
   .print_page_hash   = clockcache_print_page_hash,
};
...
clockcache_try_get_claim(clockcache *cc, uint64 entry_number)
{
   ...
   clockcache_log(0,
                  entry_number,
                  "try_get_claim: entry_number %lu claimed: %u\n",
                  entry_number,
                          ...
                          platform_status status)
{
   ...
   if (cnt % 100 == 0) {
      cnt += 1;
      platform_default_log("%s is called\n", __func__);
   }
...
#ifndef SGX_TEST
   //platform_default_log("batch=%lu, tid=%d\n", batch, platform_get_tid());
#endif
...
#ifndef SGX_TEST
   //platform_default_log("%s is done\n", __func__);
#endif
...
#ifdef SGX_TEST
      //platform_default_log("%s: was_busy=%d\n", __func__, was_busy);
      debug_assert(was_busy);
#else
      //platform_default_log("%s: outside sgx was_busy=%d\n", __func__, was_busy);
#endif
   }

#ifdef SGX_TEST
   //platform_default_log("%s start\n", __func__);
#endif
...
#ifndef SGX_TEST
         //platform_default_log("%s: clockcache_batch_start_writeback end\n", __func__);
#endif
                         ...
                         bool        blocking)
{
    ...
    */
#ifdef SGX_TEST
   while (num_passes < 10000 || blocking)
#else
   while (num_passes < 3
   ...
   {
#ifndef SGX_TEST
      //platform_default_log("%s is at %d, tid=%d\n", __func__, __LINE__, platform_get_tid());
#endif
...
#ifdef SGX_TEST
            //platform_default_log("%s is  exit at %d, tid=%d\n", __func__, __LINE__, platform_get_tid());
#endif
            return entry_no;
         }
      }
      ...
      //platform_default_log("%s is at %d, tid=%d, num_passes=%d\n", __func__, __LINE__, platform_get_tid(), num_passes);
#ifndef SGX_TEST
      //platform_default_log("%s is at %d, tid=%d\n", __func__, __LINE__, platform_get_tid());
#endif
   }
   if (blocking) {
      platform_error_log("cache locked (num_passes=%lu time=%lu nsecs)\n",
                           num_passes,
...
clockcache_try_hard_evict(clockcache *cc, uint64 addr)
{
   ...
   while (TRUE) {
      ...
      if (entry_number == CC_UNMAPPED_ENTRY) {
         clockcache_log(addr,
                        entry_number,
                        "try_hard_evict (uncached): entry %lu addr %lu\n",
                        entry_number,
                        addr);
         return;
      }
      ...
      clockcache_log(addr,
                     entry_number,
                     "try_hard_evict (cached): entry %lu addr %lu\n",
                     entry_number,
                     addr);
...
static void
clockcache_authenticate_page(clockcache *cc, page_handle *handle,
                             page_reference *disk_ref, page_type type, bool *succeed)
{
   clockcache_entry *entry = clockcache_lookup_entry(cc, handle->disk_addr);

   if (0 != memcmp(disk_ref->hash, entry->sig, HASH_SIZE)) {
      char buf[64] = { 0 };
      snprintf(buf, 64, "disk_ref--%lu\n",  handle->disk_addr);
      print_sig(disk_ref->hash, HASH_SIZE, buf);
      snprintf(buf, 64, "entry->sig-%lu\n",  handle->disk_addr);
      print_sig(entry->sig, HASH_SIZE, buf);
      platform_default_log("pid=%ld, Signature verification failed in %s for addr=%lu, entry=%p\n",
                            platform_get_tid(), __func__, handle->disk_addr, entry);

      trunk_hmac(entry->page.data, (int)clockcache_page_size(cc), buf, HASH_SIZE, type);
      print_sig(buf, HASH_SIZE, "newly-computed hmac");
...
static void
clockcache_self_authenticate_page(clockcache *cc, page_handle *handle, bool *succeed)
{
   clockcache_entry *entry = clockcache_lookup_entry(cc, handle->disk_addr);

   if (0 != memcmp(entry->page.data, entry->sig, HASH_SIZE)) {
      char buf[64] = { 0 };
      snprintf(buf, 64, "signature in page--%lu\n",  handle->disk_addr);
      print_sig(entry->page.data, HASH_SIZE, buf);
      snprintf(buf, 64, "entry->sig-%lu\n",  handle->disk_addr);
      print_sig(entry->sig, HASH_SIZE, buf);
      platform_default_log("pid=%ld, Signature verification failed in %s for addr=%lu, entry=%p\n",
                            platform_get_tid(), __func__, handle->disk_addr, entry);

      trunk_hmac(entry->page.data + HASH_SIZE, (int)clockcache_page_size(cc) - HASH_SIZE, buf, HASH_SIZE, PAGE_TYPE_FILTER);
      print_sig(buf, HASH_SIZE, "newly-computed hmac");
...
static inline bool
clockcache_page_skip_check(page_type type)
{
   platform_assert(type != PAGE_TYPE_FILTER);
   return type == PAGE_TYPE_META_PAGE || type == PAGE_TYPE_SUPERBLOCK ||
          type == PAGE_TYPE_MEMTABLE || type == PAGE_TYPE_LOCK_NO_DATA;
}

static inline bool
clockcache_sig_is_zero(char *sig)
{
   char zeros[HASH_SIZE] = {0};
   int r = memcmp(sig, zeros, HASH_SIZE);
   return r == 0;
}
...
clockcache_get_internal(clockcache   *cc,        // IN
                        page_reference *ref,     // IN
                        bool          blocking,  // IN
                        bool          skip_auth, // IN
                        page_type     type,      // IN
                        page_handle **page)      // OUT
{
   platform_assert(ref != NULL);
   uint64 addr = ref->addr;
   ...
   if (entry_number != CC_UNMAPPED_ENTRY) {
      ...
      if (!skip_auth && clockcache_sig_is_zero(&entry->sig[0])) {
         // calculate the hash
         if (type == PAGE_TYPE_FILTER) {
            r = trunk_hmac(entry->page.data + HASH_SIZE, (int)clockcache_page_size(cc) - HASH_SIZE, &entry->sig[0], HASH_SIZE, type);
            platform_assert(r == 0);
         } else if (!clockcache_page_skip_check(type)) {
            platform_assert(type != PAGE_TYPE_MEMTABLE);
            r = trunk_hmac(entry->page.data, (int)clockcache_page_size(cc), &entry->sig[0], HASH_SIZE, type);
            platform_assert(r == 0);
         }
      }
      if (cc->cfg->use_stats) {
...
clockcache_get(clockcache *cc, page_reference *ref, bool blocking, page_type type, bool *succeed)
{
   ...
   while (1) {
      retry = clockcache_get_internal(cc, ref, blocking, FALSE, type, &handle);
      if (!retry) {
         if (!clockcache_page_skip_check(type)) {
            clockcache_authenticate_page(cc, handle, ref, type, succeed);
         }
         return handle;
      }
   }
...
clockcache_get_self_auth(clockcache *cc, page_reference *ref, bool blocking, page_type type, bool *succeed)
{
   ...
   while (1) {
      retry = clockcache_get_internal(cc, ref, blocking, FALSE, type, &handle);
      if (!retry) {
         clockcache_self_authenticate_page(cc, handle, succeed);
         return handle;
      }
   }
}
...
clockcache_get_no_auth(clockcache *cc, page_reference *ref, bool blocking, page_type type)
{
   ...
   debug_assert(cc->per_thread[platform_get_tid()].enable_sync_get
                ...
                || type == PAGE_TYPE_LOCK_NO_DATA);
   while (1) {
      retry = clockcache_get_internal(cc, ref, blocking, TRUE, type, &handle);
      if (!retry) {
         return handle;
      }
   }
         ...
         case GET_RC_EVICTED:
         {
            ...
            if (iceberg_insert(&cc->lookup, lookup_no, free_entry_no))
            {
               ...
               clockcache_log(addr,
                              ...
                              entry_no,
                              addr);
               // Need to reset the hash to zero
               // for the prefetched pages
               memset(entry->sig, 0, HASH_SIZE);
            } else {
               /*
...
clockcache_page_valid(clockcache *cc, page_reference *ref)
{
   if (ref->addr % clockcache_page_size(cc) != 0)
      return FALSE;
   uint64 base_addr = clockcache_extent_base_addr(cc, ref->addr);
   if (ref->addr < allocator_get_capacity(cc->al)) {
...
clockcache_validate_page(clockcache *cc, page_handle *page, uint64 addr)
{
   page_reference ref = {.addr = addr};
   (void)ref;
   debug_assert(clockcache_page_valid(cc, &ref));
...
int clockcache_page_hash(cache *c, page_handle *page, uint64 addr)
{
   int r;
   clockcache *cc = (clockcache *)c;
   clockcache_entry *entry = clockcache_lookup_entry(cc, page->disk_addr);

   debug_assert(memcmp(page, &entry->page, sizeof(*page)) == 0);
   memset(entry->sig, 0, HASH_SIZE);
   r = trunk_hmac(page->data, (int)clockcache_page_size(cc), entry->sig, HASH_SIZE, PAGE_TYPE_INVALID);
   return r;
}

void clockcache_fill_page_reference(cache *c, char *ref,  page_handle *page)
{
   clockcache *cc = (clockcache *)c;
   clockcache_entry *entry = clockcache_lookup_entry(cc, page->disk_addr);

   memcpy(ref, &page->disk_addr, sizeof(page->disk_addr));
   memcpy(ref + sizeof(page->disk_addr), entry->sig, HASH_SIZE);
}

char* clockcache_get_page_hash(cache *c, page_handle *page)
{
   clockcache *cc = (clockcache *)c;
   clockcache_entry *entry = clockcache_lookup_entry(cc, page->disk_addr);
...
void clockcache_print_page_hash(cache *c, page_handle *page, int line)
{
   clockcache *cc = (clockcache *)c;
   clockcache_entry *entry = clockcache_lookup_entry(cc, page->disk_addr);
   debug_assert(memcmp(page, &entry->page, sizeof(*page)) == 0);
   ...
   char buf[96] = {0};
   snprintf(buf, 96, "[clockcache_print] addr=%lu caller=%d, entry=%p, status=%d", page->disk_addr, line, entry, entry->status);
   print_sig(&entry->sig[0], HASH_SIZE, buf);
}

bool enable_print = FALSE;

void set_enable_print()
{
   enable_print = TRUE;
}

void reset_enable_print()
{
   enable_print = FALSE;
}
