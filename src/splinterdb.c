...
splinterdb_config_set_defaults(splinterdb_config *cfg)
{
   ...
   if (!cfg->memtable_capacity) {
      cfg->memtable_capacity = MiB_TO_B(12);
   }
   if (!cfg->fanout) {
      cfg->fanout = 6;
   }
   if (!cfg->max_branches_per_node) {
      cfg->max_branches_per_node = 8;
   }
...
)
{
   ...
   if (kvs->spl == NULL) {
      platform_error_log("Failed to init splinter, open_existing=%d\n", open_existing);
...
splinterdb_close(splinterdb *kvs) // IN
{
   ...
   cache_print_stats(Platform_default_log_handle, kvs->spl->cc);
