...
mini_full_lock_meta_tail(mini_allocator *mini)
{
   ...
   while (1) {
      ...
      page_reference ref = { .addr = meta_tail };
      meta_page        = cache_get_no_auth(mini->cc, &ref, TRUE, mini->type);
...
mini_get_claim_meta_page(cache *cc, uint64 meta_addr, page_type type)
{
   ...
   uint64       wait = 1;
   while (1) {
      page_reference ref = { .addr = meta_addr };
      meta_page = cache_get_no_auth(cc, &ref, TRUE, type);
...
mini_deinit(cache *cc, uint64 meta_head, page_type type, bool pinned)
{
   ...
   do {
      page_reference ref = { .addr = meta_addr };
      page_handle *meta_page      = cache_get_no_auth(cc, &ref, TRUE, type);
...
mini_unkeyed_print(cache *cc, uint64 meta_head, page_type type)
{
   ...
   do {
      page_reference ref = { .addr = next_meta_addr };
      page_handle *meta_page = cache_get(cc, &ref, TRUE, type, NULL);
