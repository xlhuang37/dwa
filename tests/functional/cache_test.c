...
platform_status
test_cache_page_pin(cache *cc, page_handle **page_arr, page_reference *ref_arr, uint64 page_capacity)
{
   ...
   for (uint64 curr_page = 0; curr_page < page_capacity; curr_page++) {
      page_handle *page =
         cache_get(cc, &ref_arr[curr_page], TRUE, PAGE_TYPE_MISC, NULL);
      cache_claim(cc, page);
      cache_lock(cc, page);
      cache_pin(cc, page);
                         ...
                         uint32             extents_to_allocate)
{
   ...
   for (j = 0; j < extents_to_allocate; j++) {
      uint64 base_addr;
      ...
      for (uint32 i = 0; i < pages_per_extent; i++) {
         ...
         ref_arr[j * pages_per_extent + i].addr = addr;
         cache_hash(cc, page, addr);
         memcpy(ref_arr[j * pages_per_extent + i].hash, cache_get_page_hash(cc, page), HASH_SIZE);
         cache_unlock(cc, page);
         cache_unclaim(cc, page);
         cache_unget(cc, page);
      }
   ...
   platform_error_log("%s, j=%d, pages_per_extent=%ld\n", __func__, j, pages_per_extent);
   uint32 dirty_count1 = cache_count_dirty(cc);
   platform_error_log("%s: dirty_count1=%d\n", __func__, dirty_count1);
...
test_cache_basic(cache *cc, clockcache_config *cfg, platform_heap_id hid)
{
   ...
   ref_arr = TYPED_ARRAY_MALLOC(hid, ref_arr, pages_to_allocate);
   rc       = cache_test_alloc_extents(cc, cfg, ref_arr, extents_to_allocate);
   if (!SUCCESS(rc)) {
   ...
   uint32 dirty_count1 = cache_count_dirty(cc);
   platform_error_log("%s: dirty_count1=%d\n", __func__, dirty_count1);
   platform_error_log("%s: cfg->page_capacity=%lu\n", __func__, cfg->page_capacity);

   cache_flush(cc);
   uint32 dirty_count = cache_count_dirty(cc);
   if (dirty_count != 0) {
      platform_error_log("(xxx) Expected no dirty entries but found: %u, dirty_count1=%u, extents_to_allocate=%u\n",
                         dirty_count, dirty_count1, extents_to_allocate);
      rc = STATUS_TEST_FAILED;
      goto exit;
   }
   ...
   for (uint32 j = 0; j < pages_allocated && SUCCESS(rc);) {
      ...
      for (i = 0; i < cfg->page_capacity; i++) {
         uint64 page_idx = j + i;
         page_reference ref = ref_arr[page_idx];
         page_arr[i] = cache_get(cc, &ref, TRUE, PAGE_TYPE_MISC, NULL);
   ...
   if (dirty_count != 0) {
      platform_error_log("(1) Expected no dirty entries but found: %u",
                         dirty_count);
      rc = STATUS_TEST_FAILED;
      goto exit;
   }
   ...
   for (uint32 j = 0; j < pages_allocated && SUCCESS(rc);) {
      uint32 i;
      for (i = 0; i < cfg->page_capacity; i++) {
         page_reference ref = ref_arr[j+i];
         page_arr[i] = cache_get(cc, &ref, TRUE, PAGE_TYPE_MISC, NULL);
   ...
   if (dirty_count != 0) {
      platform_error_log("(2) Expected no dirty entries but found: %u",
                         dirty_count);
      rc = STATUS_TEST_FAILED;
      goto exit;
   }
   ...
   dirty_count = cache_count_dirty(cc);
   if (dirty_count != 0) {
      platform_error_log("(3) Expected no dirty entries but found: %u",
                         dirty_count);
      rc = STATUS_TEST_FAILED;
      goto exit;
   }

   rc = test_cache_page_pin(cc, page_arr, ref_arr, cfg->page_capacity);

   /*
    * Deallocate all the entries.
    */
   for (uint32 i = 0; i < extents_to_allocate; i++) {
      uint64     addr = ref_arr[i * pages_per_extent].addr;
      allocator *al   = cache_allocator(cc);
                       ...
                       cache_test_index_itor *itor)
{
   ...
   for (uint32 i = 0; i < cfg->page_capacity; i++) {
      ...
      page_reference ref = ref_arr[idx];
      page_handle *ph  = cache_get(cc, &ref, TRUE, PAGE_TYPE_MISC, NULL);
   ...
   uint32 dirty_count = cache_count_dirty(cc);
   if (dirty_count != 0) {
      platform_error_log("(5) Expected no dirty entries but found: %u",
                         dirty_count);
      rc = STATUS_TEST_FAILED;
   }
                 ...
                 uint64             al_extent_capacity)
{
   ...
   rc = cache_test_dirty_flush(cc, cfg, "Seq", ref_arr, &itor);
   if (!SUCCESS(rc)) {
   ...
   rc = cache_test_dirty_flush(cc, cfg, "Reverse Seq", ref_arr, &itor);
   if (!SUCCESS(rc)) {
   ...
   rc = cache_test_dirty_flush(cc, cfg, "Hop", ref_arr, &itor);
   if (!SUCCESS(rc)) {
      platform_error_log("failed test seq dec");
      goto exit;
   }
   ...
   rc = cache_test_dirty_flush(cc, cfg, "Random", ref_arr, &itor);
   if (!SUCCESS(rc)) {
      platform_error_log("failed test seq dec");
      goto exit;
   }
...
typedef struct {
   ...
   const page_reference *ref_arr;                // IN array of page addrs
                        ...
                        bool         was_async[])
{
   ...
   const page_reference *ref_arr   = params->ref_arr;
   cache        *cc         = params->cc;
   uint64        j;
   ...
   for (j = 0; j < batch_end; j++) {
      ...
      cache_assert_ungot(cc, ref_arr[batch_start + j].addr);
   }
}
...
test_do_read_batch(threadid tid, test_params *params, uint64 batch_start)
{
   ...
   const page_reference *ref_arr   = &params->ref_arr[batch_start];
   ...
   for (j = 0; j < READER_BATCH_SIZE; j++) {
      ...
      cache_assert_ungot(cc, ref_arr[j].addr);
      ...
      {
         page_reference ref = ref_arr[j];
         ctxt->page = cache_get(cc, &ref, TRUE, PAGE_TYPE_MISC, NULL);
         res        = async_success;
      } else {
         page_reference ref = ref_arr[j];
         res = cache_get_async(cc, &ref, PAGE_TYPE_MISC, ctxt);
      }
...
test_reader_thread(void *arg)
{
   ...
   for (i = k = 0; i < num_pages; i += READER_BATCH_SIZE) {
      ...
      if (i >= k + params->num_pages_ws) {
         for (j = 0; j < READER_BATCH_SIZE; j++) {
            ...
            handle_arr[k + j] = NULL;
            cache_assert_ungot(cc, ref_arr[k + j].addr);
         }
         k += READER_BATCH_SIZE;
      }
      bool need_retry;
   ...
   for (k = 0; k < num_pages; k++) {
      cache_assert_ungot(cc, ref_arr[k].addr);
   }
}
...
test_writer_thread(void *arg)
{
   ...
   for (i = k = 0; i < num_pages; i++) {
      ...
      do {
         page_reference ref = ref_arr[i];
         handle_arr[i] = cache_get(cc, &ref, TRUE, PAGE_TYPE_MISC, NULL);
...
cache_test(int argc, char *argv[])
{
   ...
   if (benchmark) {
      ...
      platform_default_log("testt_cache_basic is called\n");
      rc = test_cache_basic(ccp, &cache_cfg, hid);
      platform_default_log("testt_cache_basic is done\n");
   }
   platform_assert_status_ok(rc);
